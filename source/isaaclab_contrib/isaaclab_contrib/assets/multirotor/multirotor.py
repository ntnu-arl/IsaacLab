...
if TYPE_CHECKING:
    ...
    from .multirotor_newton import MultirotorNewton
    from .multirotor_physx import MultirotorPhysx
...
class Multirotor:
    ...
    _backend_class_names: dict[str, str] = {
        "physx": "MultirotorPhysx",
        "newton": "MultirotorNewton",
    }
    """Concrete multirotor class names keyed by physics backend."""

    def __new__(cls, cfg: MultirotorCfg) -> MultirotorPhysx | MultirotorNewton:
        """Create the multirotor implementation for the active physics backend.

        Args:
            cfg: A multirotor configuration instance.

        Returns:
            The backend-specific multirotor instance.

        Raises:
            ValueError: If the active physics backend has no multirotor implementation.
        """
        if cls is not Multirotor:
            return object.__new__(cls)

        backend = Articulation._get_backend()
        try:
            class_name = cls._backend_class_names[backend]
        except KeyError:
            supported_backends = ", ".join(sorted(cls._backend_class_names))
            raise ValueError(
                f"Multirotor does not support physics backend {backend!r}. Supported backends: {supported_backends}."
            ) from None

        module = importlib.import_module(f"{__package__}.multirotor_{backend}")
    ...
    def _create_multirotor_data(self) -> MultirotorData:
