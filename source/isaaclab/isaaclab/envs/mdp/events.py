...
from isaaclab.assets import Articulation, DeformableObject, FixedWing, RigidObject
...
def randomize_rigid_body_scale(
    ...
    if isinstance(scale_range, dict):
        ...
        rand_samples = math_utils.sample_uniform(
            *scale_range, (len(env_ids), 1), device="cpu"
        )
    ...
    with Sdf.ChangeBlock():
        for i, env_id in enumerate(env_ids):
            ...
            if not has_scale_attr:
                op_order_spec = prim_spec.GetAttributeAtPath(
...
class randomize_fixed_wing_aerodynamic_parameters(ManagerTermBase):
    """Randomize aerodynamic coefficients on a fixed-wing articulation.

    This term updates the shared fixed-wing configuration, so the sampled values apply to all
    environments in lockstep. It is best used when you want one randomized vehicle model per run or
    when you reset all environments together.
    """
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        self.asset_cfg: SceneEntityCfg = cfg.params["asset_cfg"]
        self.asset: FixedWing = env.scene[self.asset_cfg.name]

        if not isinstance(self.asset, FixedWing):
            raise ValueError(
                "Randomization term 'randomize_fixed_wing_aerodynamic_parameters' is only supported for "
                f"FixedWing assets, but got: '{type(self.asset)}'."
            )

        if cfg.params["operation"] == "scale":
            for name, params in cfg.params.get("wing_parameter_ranges", {}).items():
                _validate_scale_range(
                    params, f"wing_parameter_ranges['{name}']", allow_zero=False
                )
            for name, params in cfg.params.get("engine_parameter_ranges", {}).items():
                _validate_scale_range(
                    params, f"engine_parameter_ranges['{name}']", allow_zero=False
                )
            if "rho_distribution_params" in cfg.params:
                _validate_scale_range(
                    cfg.params["rho_distribution_params"],
                    "rho_distribution_params",
                    allow_zero=False,
                )
            if "wing_drag_tensor_range" in cfg.params:
                _validate_scale_range(
                    cfg.params["wing_drag_tensor_range"],
                    "wing_drag_tensor_range",
                    allow_zero=False,
                )
        elif cfg.params["operation"] not in ("abs", "add"):
            raise ValueError(
                "Randomization term 'randomize_fixed_wing_aerodynamic_parameters' does not support operation: "
                f"'{cfg.params['operation']}'."
            )

        self._base_rho = float(self.asset.cfg.rho)
        self._base_wing_params = {
            wing_name: {
                name: float(getattr(wing_cfg, name))
                for name in cfg.params.get("wing_parameter_ranges", {})
            }
            for wing_name, wing_cfg in self.asset.cfg.wings.items()
        }
        self._base_engine_params = {
            engine_name: {
                name: float(getattr(engine_cfg, name))
                for name in cfg.params.get("engine_parameter_ranges", {})
            }
            for engine_name, engine_cfg in self.asset.cfg.engines.items()
        }
        self._base_wing_drag_tensor = {
            wing_name: drag_tensor.clone()
            for wing_name, drag_tensor in self.asset._aerodata.wing_drag_tensor.items()
        }
    ...
    def _randomize_scalar(
        ...
        data = torch.tensor([[value]], device="cpu")
        randomized = _randomize_prop_by_op(
            data, distribution_params, None, slice(None), operation, distribution
        )
        return float(randomized.item())
    ...
    def __call__(
        ...
        env_ids: torch.Tensor | None,
        asset_cfg: SceneEntityCfg,
        wing_parameter_ranges: dict[str, tuple[float, float]] | None = None,
        engine_parameter_ranges: dict[str, tuple[float, float]] | None = None,
        rho_distribution_params: tuple[float, float] | None = None,
        wing_drag_tensor_range: tuple[float, float] | None = None,
        ...
        if wing_parameter_ranges is not None:
            for wing_name, wing_cfg in self.asset.cfg.wings.items():
                for name, params in wing_parameter_ranges.items():
                    if hasattr(wing_cfg, name):
                        value = self._base_wing_params[wing_name][name]
                        setattr(
                            wing_cfg,
                            name,
                            self._randomize_scalar(
                                value, params, operation, distribution
                            ),
                        )

        if engine_parameter_ranges is not None:
            for engine_name, engine_cfg in self.asset.cfg.engines.items():
                for name, params in engine_parameter_ranges.items():
                    if hasattr(engine_cfg, name):
                        value = self._base_engine_params[engine_name][name]
                        setattr(
                            engine_cfg,
                            name,
                            self._randomize_scalar(
        ...
        if rho_distribution_params is not None:
            self.asset.cfg.rho = self._randomize_scalar(
                self._base_rho, rho_distribution_params, operation, distribution
            )

        if wing_drag_tensor_range is not None:
            for wing_name, drag_tensor in self.asset._aerodata.wing_drag_tensor.items():
                scale = self._randomize_scalar(
                    1.0, wing_drag_tensor_range, operation, distribution
                )
                self.asset._aerodata.wing_drag_tensor[wing_name] = (
                    self._base_wing_drag_tensor[wing_name] * scale
                )
        ...
        for actuator in self.asset.actuators.values():
            ...
            if stiffness_distribution_params is not None:
                ...
                stiffness[:, actuator_indices] = (
                    self.asset.data.default_joint_stiffness[env_ids][
                        :, global_indices
                    ].clone()
                )
            ...
            if damping_distribution_params is not None:
                ...
                damping[:, actuator_indices] = self.asset.data.default_joint_damping[
                    env_ids
    ...
    def __call__(
        ...
        if self.asset_cfg.joint_ids == slice(None):
            ...
            joint_ids = torch.tensor(
                self.asset_cfg.joint_ids, dtype=torch.int, device=self.asset.device
            )
        ...
        if friction_distribution_params is not None:
            ...
            if major_version >= 5:
                ...
                dynamic_friction_coeff = torch.minimum(
                    dynamic_friction_coeff, friction_coeff
                )

                # Index once at the end
                dynamic_friction_coeff = dynamic_friction_coeff[
                    env_ids[:, None], joint_ids
                ]
                viscous_friction_coeff = viscous_friction_coeff[
                    env_ids[:, None], joint_ids
                ]
            else:
...
class randomize_fixed_tendon_parameters(ManagerTermBase):
    ...
    def __call__(
        ...
        if (
            ...
            self.asset.set_fixed_tendon_position_limit(
                tendon_limits, tendon_ids, env_ids
            )
...
def reset_scene_to_default(
    env: ManagerBasedEnv, env_ids: torch.Tensor, reset_joint_targets: bool = False
):
...
class randomize_visual_texture_material(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        version = re.match(r"^(\d+\.\d+\.\d+)", rep.__file__.split("/")[-5][21:]).group(
            1
        )
    ...
    def __call__(
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            texture_paths = (
                texture_paths
                if texture_paths
                else self._cfg.params.get("texture_paths")
            )
            texture_rotation = (
                texture_rotation
                if texture_rotation
                else self._cfg.params.get("texture_rotation", (0.0, 0.0))
            )
            ...
            random_textures = self.texture_rng.generator.choice(
                texture_paths, size=num_prims
            )
            ...
            rep.functional.modify.attribute(
                self.material_prims, "diffuse_texture", random_textures
            )
            rep.functional.modify.attribute(
                self.material_prims, "texture_rotate", random_rotations
            )
...
class randomize_visual_color(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            stage = get_current_stage()
            prims_group = rep.functional.get.prims(
                path_pattern=mesh_prim_path, stage=stage
            )
    ...
    def __call__(
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            num_prims = len(self.material_prims)
            random_colors = self.color_rng.generator.uniform(
                colors[0], colors[1], size=(num_prims, 3)
            )

            rep.functional.modify.attribute(
                self.material_prims, "diffuse_color_constant", random_colors
            )
